...
# Base code for this model is adapted from the GRec model.
# Reference: B. Li et al., “Embedding app-library graph for neural third party library recommendation,” 
# in Proc. 29th ACM Joint Meeting Eur. Softw. Eng. Conf. Symp. Found. Softw. Eng. (ESEC/FSE), 2021, pp. 466–477.
...
class Model_Wrapper(object):
    def __init__(self, data_config, pretrain_data):
        ...
        if self.alg_type in ['MsRec']:
            self.model = MsRec(self.n_users, self.n_items, self.emb_dim, self.layer_num, self.mess_dropout)
        else:
            raise Exception('Dont know which model to train')

        self.optimizer = optim.Adam(self.model.parameters(), lr=self.lr)

        self.norm_u1, self.norm_u2, self.norm_i1, self.norm_i2,self.adj_cat,self.adj_cat_user = self.MultiScale_hyper_edge(
    ...
    def MultiScale_hyper_edge(self, file):
